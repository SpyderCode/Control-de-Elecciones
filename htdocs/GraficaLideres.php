...
<?php 
require "Connect.php";
$query="select lideres.id,lideres.Nombre,lideres.paterno,(SELECT COUNT(* ) from promovidos,promotores WHERE (promotores.id_lider=lideres.id) AND (promotores.id=promovidos.id_promotor) order by Count(*) DESC LIMIT 10) as promovidos FROM lideres;";
    //"select promotores.id,promotores.Nombre,count(promovidos.id_promotor) as Promovidos from promotores LEFT JOIN promovidos on (promotores.id=promovidos.id_promotor) GROUP BY promotores.id ORDER by count(promovidos.id_promotor) DESC limit 10;";
...
<body onload="checksession()">
	...
	<div class="primary_header">
	    <h1 class="title">GRÁFICA DE LIDERES</h1>
		</div>
...
<script type="text/javascript">
 ...
 var options = {
          chart: {
            title: 'Top De Líderes',
            subtitle: 'Los líderes con mayor número de promovidos',
          },
...
function download(){
    ...
    saveimage(image64,"GraficaLideres");    
    });
